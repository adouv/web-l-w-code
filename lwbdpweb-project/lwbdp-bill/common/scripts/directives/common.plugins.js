 ...
 * 上拉刷新，下拉加载指令
 */
export default angular.module('lw.plugin', []).directive("lwLoadRefresh", ["serviceUtil", function (serviceUtil) {
    return {
        restrict: "E",
        transclude: true,
        replace: true,
...
}]).directive("lwPagination", ['serviceUtil', function (serviceUtil) {
    return {
        restrict: "E",
        replace: true,
...
}]).directive('lwPaging', ['serviceUtil', function (serviceUtil) {
    return {
        restrict: 'E',
        replace: true,
...
}).directive('lwDownLoad', ['serviceUtil', function (serviceUtil) {
    return {
        scope: {},
        link: function (scope, elem, attrs) {
            elem.on('click', function () {
                var url = attrs.lwDownLoad, path = sys_config.path;
...
}]).directive("lwPage", ['serviceUtil', function (serviceUtil) {
	return {
		restrict: "E",
		replace: true,
		template: '<div id="lwPage" style="position: relative"></div>',
		link: function (scope, elem, attrs) {
			scope.condition = scope.$parent.condition = {};
			...
			scope.$parent.toSearch = scope.toSearch = searchKeyword;
			scope.$parent.pageSearch = scope.pageSearch = searchKeyword;
			...
			function getList(callback) {
				var param = $.extend({},params, scope.condition);
				serviceUtil.requestServer(attrs.url, 'get', param,function (data,headers) {
					scope.$parent.headerCount = scope.header = headers['x-record-count'];
          scope.$parent.headerAmount = scope.headerAmount = headers['x-record-amount'];
					var total = headers['x-record-count'];
					scope.$parent.datas = scope.datas = data;

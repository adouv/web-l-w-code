...
(function() {
  ...
  angular.module('ImageCropper',[])
    .directive('imageCrop', function() {
      ...
      return {
        template: '<div class="image_cropper"><div id="image-crop-{{ rand }}" class="ng-image-crop ng-image-crop--{{ shape }}" ng-style="moduleStyles"><section ng-style="sectionStyles"><canvas class="cropping-canvas" width="{{ canvasWidth }}" height="{{ canvasHeight }}" ng-mousemove="onCanvasMouseMove($event)" ng-mousedown="onCanvasMouseDown($event)"></canvas><div ng-style="croppingGuideStyles" class="cropping-guide"></div></section></div><div class="img_plus_narrow"><span class="connectIconfont icon-connect-plus" ng-click="enlarge()"></span><div class="zoom-handle" ng-mousemove="onHandleMouseMove($event)" ng-mousedown="onHandleMouseDown($event)" ng-click="onClickZoom($event)" ng-mouseup="onHandleMouseUp($event)"><span id="blue-circle" class="blue-circle"></span></div><span class="connectIconfont icon-connect-minus"  ng-click="narrow()"></span></div><div>',
        replace: true,
        restrict: 'AE',
        scope: {
		  crop: '=',
        ...
        link: function (scope, element, attributes) {
          ...
          function zoomImage(val) {
            var proposedZoomLevel = to2Dp(zoom + val*1);
            if(proposedZoomLevel<maxZoomedInLevel){
                proposedZoomLevel = maxZoomedInLevel;
            }
            ...
            let circle = document.querySelector("#blue-circle");
            let num = zoom-1;
            num  = num > 0.93 ? 0.93 : num;
            circle.style.left = num*100+'%';
			...
			if(scope.src) {
				if(scope.step != 3) {
					if(typeof(scope.src) == 'Blob') {
                        ...
                        if(scope.src.indexOf("lw-fileserver")>-1){
                            var tocanvas = document.createElement('CANVAS'),
                            toctx = tocanvas.getContext('2d'),
                            img = new Image;
                            ...
                            img.onload = function(){
                                ...
                                var dataURL = tocanvas.toDataURL();
                                loadImage(dataURL);
                                tocanvas = null; 
                            };
                            img.src = scope.src;
          ...
          scope.onClickZoom = function(e){
              e.preventDefault();
              let left = e.layerX;
              let circle = document.querySelector("#blue-circle");
              let originLeft = circle.style.left ? circle.style.left.replace("%",'') : 0;
              let num = parseFloat( left/140).toFixed(2);
              let distance = ((num*100-originLeft)/100).toFixed(2);
              zoomImage(distance);
          }

...
export class CoursePreparePage implements OnInit {
	...
	setDesignCode() {
		if (this.selectExerciseIds.length === 0) {
			return;
		}
	...
	isHaveUpload(callback?: Function, errCallback?: Function) {
		// let isDown = false;
		// console.log(this.coursewareHeader.downloadList);
		// this.coursewareHeader.downloadList.map((data)=>{
		// 	if(data.downloading){
		// 		isDown = true;
		// 	}
		// })
		if (!this.isUploadCompelte) {
			...
			this.dialogService.openConfirm({
				...
				onOk: () => {
					this.coursewareHeader.cancelAll();
					// this.coursewareHeader.downloadList = [];
					if (callback) {
						callback();
					}
					// this.coursewareHeader.cancelAll();

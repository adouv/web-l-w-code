...
export default class auditOpinion{
    constructor(ProjectInterface,WorkflowInterface,$sessionStorage){
        this.ProjectInterface = ProjectInterface;
        this.WorkflowInterface = WorkflowInterface;
        this.getAuditOpinion('agree_suggestion',true);
     ...
     * @param result
     */
    getAuditOpinion(result,isNoNextAuditor) {
        this.ProjectInterface.getAuditOpinionCateGory(result).then(res=>{
            this.classifyOpinions = res.data;
        });
        !isNoNextAuditor&&this.toggleResult();
    }
    ...
    getTemplate(){
        return `<form class="w5c-form" novalidate name="purchaseAuditForm">
            ...
            <div class="audit_row">
                ...
                <div class="col_div" style="width:600px;">
                    <p class="audit_result">
                        <label><input class="radio_class" type="radio" ng-model="purchaseForm.editForm.approved" ng-change="purchaseForm.getAuditOpinion('agree_suggestion')" value="true">通过</label>
                        <label><input class="radio_class" type="radio" ng-model="purchaseForm.editForm.approved" ng-change="purchaseForm.getAuditOpinion('disagree_suggestion')" value="false">不通过</label>
                    </p>
            ...
            <div class="audit_row">
                <p class="col_span">审核意见归类（非必填）：</p>
                <p class="col_div audit_span audit_result">
                   <label ng-repeat="opinion in purchaseForm.classifyOpinions"><input class="radio_class" type="radio" ng-model="purchaseForm.editForm.audit_reason" ng-value="opinion.itemValue">{{opinion.itemName}}</label>
                </p>
            </div>
            <div class="audit_row">
...
auditOpinion.$inject = ['ProjectInterface','WorkflowInterface','$sessionStorage'];

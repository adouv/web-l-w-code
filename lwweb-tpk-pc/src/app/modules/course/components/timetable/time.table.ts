		...
		if (this.dialog) {
			...
			this.items.map((item) => {
				...
				if (item.data && item.data.id && item.data.id != '') {
					...
					let findIndex: number = this.selectList.findIndex((sl) => { return sl.lessonId === item.data.id && sl.date == item.data.date });
					// return sl.lessonId === item.data.id && sl.date == item.data.date;
					item.data.status = (findIndex > -1);
				}
			});
		}
	...
	getStyle(index) {
		...
		switch (state) {
			...
			case ClassSheetEntity.PREPAREING_LESSON:
				style = { 'background-color': this.tabType == 1 ? '#77879C' : '#226cfb' };
				break;

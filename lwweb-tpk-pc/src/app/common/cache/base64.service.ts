...
export class Base64Service {
	...
	decode(s) {
		if(s.includes('?')) {
			let gardenId;
			gardenId =  s.split('?')[0].split('=')[1];
			s = s.split('?')[1].split('loginfo=')[1];
			localStorage.setItem('s_gardenId', gardenId);
		}
		s = s.replace(/\s/g, '');
		if (s.length % 4) {
	...
	encode(s) {
		const buffer = this.toUtf8(s),
			len = buffer.length, enc = [, , ,];
		let result, nan0, nan1, position = -1, nan2;
	...
	urlEncode(input) {
		const output = this.encode(input);
		return output
			.replace(/\+/g, '-')
			.replace(/\//g, '_')
			.split('=', 1)[0];

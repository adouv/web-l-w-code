	...
	ngOnInit() {
		...
		if (this.playType == undefined && !this.hasInit) {
			...
			if (this.source && this.source.length > 0) {
				let type = this.getSuffixName(this.source[0].pcPlayUrl);
	...
	private createVideo(from) {
		...
		if (this.videoNum == 1) {
			...
			<div style="width:100%;height:100%;float:left;border:1px solid #333333;" id="subContainer0">
				...
				<span style="color: #999999; margin-top: 15px; font-size: 15px">视频源错误</span>
				</div>
			</div>
		 </section>'`);
			...
			<section style="width:100%;height:100%;   position: absolute;top: 0;display:flex;align-items:center;" *ngIf="videoNum==2">
			   <div style="width:49.6053%;height:100%;float:left;margin-right:0.7894%;overflow: hidden;border:1px solid #333333;" id="subContainer0" >
			   ...
			   <div style="float:left;width:49.6053%;height:100%;overflow: hidden;border:1px solid #333333;" id="subContainer1">
...
*/
	...
	isReady(videoNum) {
		...
		if (videoNum == 3) {
			let player2 = videojs(this.idArr[2], options, function onPlayerReady() {
				...
				this.on("error", function (e) {
					console.log("播放器监听到错误...>1", e);
					that.errorPage = true;
					$("#" + outerSelf.idArr[2]).css("display", "none");
					$("#" + outerSelf.idArr[2]).next().css("display", "flex");
				});
				//该函数需要在设备准备好的时候调用
				outerSelf.hideAllControls();
			});
		...
		let bigPlayer = videojs(this.idArr[0], options, function onPlayerReady() {
			...
			outerSelf.setLiveRatio(videoNum - 1);

			$(".vjs-text-track-display").css("pointer-events", "visible");
			let self = this;
			let index = videoNum - 1;

  ...
  <div class="menu">
    ...
    >
      <i
        ...
        @click="openWin({key:99,label:'笔',icon:'',show:true,url:'noteLists',options:{fullscreen: true}});"
      ></i>
...
export default {
  ...
  async mounted() {
    ...
    // this.kfk$.start((dto, len) => {
    //   console.log(dto, len);
    // });

    this.udp$.udpService(8486, (data, lwUdp, len) => {
   
      let win_id_sub = this.local$.getItem("win_id_subjectiveProblem");
      let win_id_note = this.local$.getItem("win_id_noteList");
      let win_id_taking=this.local$.getItem("win_id_noteTaking");
     
      if (win_id_sub !== "") {
        myThis.win$
          .getRemote()
          .BrowserWindow.fromId(parseInt(win_id_sub))
      ...
      if (win_id_note !== "") {
        myThis.win$
          .getRemote()
          .BrowserWindow.fromId(parseInt(win_id_note))
          .webContents.send("penData", {
            data: data,
            lwUdp: lwUdp,
            len: len
          });
      ...
      if(win_id_taking!==""){
                myThis.win$
          .getRemote()
          .BrowserWindow.fromId(parseInt(win_id_taking))
          .webContents.send("penData", {
            data: data,
            lwUdp: lwUdp,
            len: len

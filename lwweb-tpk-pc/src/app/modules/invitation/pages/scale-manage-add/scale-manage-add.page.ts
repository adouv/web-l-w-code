...
export class ScaleMangeAddPage implements OnInit {
	...
	selectedFileOnChanged(e) {
		...
		if (this.allowTypes.indexOf(fileExt) > -1 && queueLists.length > 0) {
			...
			queueLists[queueLists.length - 1].onSuccess = (response, status, headers) => {
				if (status === 200) {
					this.modelTemplate = this.getSubstr(JSON.parse(response));
	...
	// 截取字符串
	getSubstr(list) {
		for (const data of list.items) {
			const len = data ? data.name.length : 0;
			data.name = len > 10 ? data.name.substr(0, 10) : data.name;
			for (const child of data.childItems) {
				const nameLen = child ? child.name.length : 0;
				child.name = nameLen > 100 ? child.name.substr(0, 100) : child.name;
			}
		}
		return list;
	}

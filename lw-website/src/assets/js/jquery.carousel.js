...
;(function($){
    var Caroursel = function (caroursel){
        ...
        this.posterItems = caroursel.find(".poster-items");
        this.length = caroursel.find(".poster-items").length;
        this.index = 0;
        ...
        this.prevBtn.bind("click",function(){
            if(self.rotateFlag){
                ...
                for(var i = 0, len = self.length ; i < len ; i++){
                    self.caroursel.find(".poster-items").eq(i).removeClass('active');
                };
                self.caroursel.find(".poster-items").eq(self.index%self.length < 0 ? self.length + self.index % self.length :  self.index % self.length).addClass('active');
    ...
    Caroursel.prototype = {
        ...
        setFirstPosition:function(){
            ...
            //设置两个按钮的样式
            this.prevBtn.css({"width":width , "height":this.setting.height});
            this.nextBtn.css({"width":width , "height":this.setting.height});

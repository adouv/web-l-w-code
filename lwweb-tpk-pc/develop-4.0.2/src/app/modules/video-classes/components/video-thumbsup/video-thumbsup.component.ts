...
@Component({
	...
	template: `<button class="round-btn btn-zan-default btn-zan" nz-button [nzType]="'danger'" (click)="changeFavour()">
		...
		<!--
		<footer flex-box="1">
			<button class="round-btn btn-zan-default btn-zan" [disabled]="(videoDetails.status==0||(videoDetails.status!=0&&videoDetails.subjectName=='自习'))" nz-button [nzType]="'danger'" (click)="changeFavour()">
				<i class="iconfont" [ngClass]="{'icon-dianzan':isFavour,'icon-dianzanmoren':!isFavour}"></i>
				<span>赞</span>
				<p>{{favourList.length}}</p>
			</button>

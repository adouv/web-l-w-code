...
window.UE.Editor.prototype.getKfContent = function(callback){

    var me = this;
    var actionUrl = me.getActionUrl(me.getOpt('scrawlActionName')),
        params = window.UE.utils.serializeParam(me.queryCommandValue('serverparam')) || '',
        url = window.UE.utils.formatUrl(actionUrl + (actionUrl.indexOf('?') == -1 ? '?':'&') + params);

    // 找到所有的base64
    var count = 0;
    var imgs =me.body.getElementsByTagName('img');
    var base64Imgs = [];
    window.UE.utils.each(imgs, function(item){
    ...
    if (base64Imgs.length == 0){
        ...
        window.UE.utils.each(base64Imgs, function(item){

            var opt ={};
            opt[me.getOpt('scrawlFieldName')]= item.getAttribute('src').replace(/^[^,]+,/, '');
            opt.onsuccess = function(xhr){
                var json = window.UE.utils.str2json(xhr.responseText),
            ...
            window.UE.ajax.request(url, opt);

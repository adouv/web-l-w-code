import './style.css';
export default angular.module('msg', []).factory("dialogsManager", function ($q, $compile, $timeout, $rootScope) {
    ...
    let confirmTmp = `<div class='nspop_megcontainer myactive' ng-click="close($event)">
                        <div class='lw-confirm-wrap'>
                            <div class="lw-confirm-header">
                                {{title}}
                                <span class="lw-close iconfont">&#xe651;</span>
                            </div>
    ...
    function createConfirm(options) {
        ...
        megPromise.then((result) => {
            ...
            if(options.callback&&angular.isFunction(options.callback)){
                ...
                &&(angular.isFunction(options.callback[0])
                ||angular.isFunction(options.callback[1]))){
                let callback = [];
                callback[0] = ()=>{

...
export default {
  ...
  async penArgsAnalysis(args, callback) {
    ...
    if (args.length > 20) {
      ...
      Vue.sql.insert('lw_pen_problem', problemDto);
    } else {
      ...
      Vue.sql.insert('lw_pen_notes', penDto);
    };

    // let item = {
    //   penid: problemDto.penid,
    //   status: -1
    // };
    // if (args.length > 20) {
    //   item.status = 1;
    // } else {
    //   if (penDto.pressure !== 0) {
    //     item.status = 1;
    //   } else {
    //     item.status = 0;
    //   }
    // }
    // let find = penList.find(i => i.penid === item.penid);
    // if (find !== undefined) {
    //   penList.push(item);
    // } else {

...
export default class uploadControllerMaterial extends baseForm{
    constructor($scope){
        super();
        this.$scope = $scope;
    }
    ...
    getTemplate(){
        return `<div class="repaDisposal_info" ng-if="repairForm.isEdit===true||repairForm.isEdit===undefined">
            ...
            <div class="repaDisposal_info" ng-if="repairForm.isEdit===false">
                <div class="disposal_content">
                    <div class="reDraft_list">
                        <span class="w250 word_center fl">项目造价材料电子版：</span>
                        <div class="bale_list fl">
                            <pic-view ng-repeat="x in repairForm.detailForm.attachments track by $index" ng-if="x.type == 'projectCost'" file-name="x.name" file-path="x.url"></pic-view>
                        </div>
                    </div>
                    <div class="reDraft_list">
                        <span class="w250 word_center fl">控制价确认单原件扫描件：</span>
                        <div class="bale_list fl">
                            <pic-view ng-repeat="x in repairForm.detailForm.attachments track by $index" ng-if="x.type == 'control'" file-name="x.name" file-path="x.url"></pic-view>
                        </div>
                    </div>
...
uploadControllerMaterial.$inject = ['$scope'];

 ...
 (function($){
	...
	$.extend($.lwPageRenderers.defaultRenderer.prototype, {
		...
		getLinks:function(current_page, eventHandler,psize) {
			...
			if(this.opts.showSelect)fragment.append("<select class='tm_psize_go'><option value='5'>5</option><option value='10'>10</option><option value='12'>12</option><option value='15'>15</option><option value='20'>20</option></select>&nbsp;");
			if(this.opts.showGo)fragment.append("<a class='countNum' href='javascript:void(0);'>共<label class='tmui_page_itemcount'>"+this.maxentries+"</label>条</a><input type='text' title='请输入其他页码'  class='tm_number' value='"+proxySzie+"' id='tm_pagego'/>&nbsp;<a href='javascript:void(0);' title='请输入其他页码' style='' class='tm_go'>GO</a>");
			return fragment;
	...
	$.fn.lwPage = function(maxentries, opts){
		...
		function selectPage_psize(new_current_page,psize) {
			...
			// call the callback and propagate the event if it does not return false
			console.log(new_current_page,psize)
    ...
    var ishow = !maxentries;
        ...
        np = Math.ceil(maxentries/opts.items_per_page);
        // console.log(opts)
				var goPage = $("#tm_pagego").val()*1;
